...
class CNN(nn.Module):
    """ A subclass of nn.Module to define different layers of the convolutional neural network
        Methods: 
             __init__: initialize the Convolutional neural network layers.
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(400, 120)  # an affine operation: y = Wx + b
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 10)

    def forward(self, x):
        # x is originally Nx1x28x28
        x = F.pad(x, (2,2,2,2))
        # x is padded to Nx1x32x32
        x = F.relu(F.max_pool2d(self.conv1(x), 2)) 
        # x becomes Nx10x24x24 after the first convolution and Nx6x14x14 after the max pooling
        x = F.relu(F.max_pool2d(self.conv2(x), 2))
        # x becomes Nx20x8x8 after the second convolution and Nx16x5x5 after the max pooling
        x = torch.flatten(x, 1)
        # x becomes Nx320 after flatenning
        x = F.relu(self.fc1(x))
        # x becomes Nx120 after the second linear function
        x = F.relu(self.fc2(x))
        # x becomes Nx84 after the first linear function
        x = self.fc3(x)
        # x becomes Nx10 after the second linear function
        return x
...
def train_loop(dataloader, model, loss_fn, optimizer):
    """ Train the model with one given epoch
        Arguments: 
            ...
            model: The defined convolutional neural network model.
...
if __name__ == '__main__':
    ...
    plt.title('ConvolutionalNN Learning Curve (loss)')
    ...
    plt.title('ConvolutionalNN Learning Curve (accuracy)')
    ...
    plt.title('ConvolutionalNN Accuracy in Classification of Digits')
    plt.xlabel('Numbers')
    plt.ylabel('Classification accuracy')
    plt.xticks(range(10))
    ax.bar_label(bars)
    ...
    plt.title('Loss')
    plt.suptitle('ConvolutionalNN Box and Whisker Plots')
    plt.show()
